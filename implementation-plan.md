...
- **CON-006**: Migrations, generated database types, database fixtures/helpers, `package.json`, lockfile, shared configuration, and shared global styles are serialization points and must not be edited concurrently. A migration-owning task retains exclusive ownership through empty replay, generated-type review, and recorded green workflow evidence. As a narrow completion exception, `TASK-010A` may also edit `.github/workflows/ci.yml` only to add its generated-types stale check and downloadable generated-types artifact; this workflow edit remains serialized with all other workflow ownership.
...
- **CON-009**: Automated database and concurrency tests must target a CI-hosted disposable Supabase stack in GitHub Actions, never local Docker and never the shared remote development project. Applying an already reviewed migration to a remote development/demo project is a separate deployment operation, not an automated-test substitute.
...
The initial schema remains limited to Auth and `sellers` except for the narrowly scoped login-rate-limit table/function introduced by `TASK-010A`; commerce tables remain prohibited until Phase 2. Completed foundation tasks are recorded first. `TASK-007A` records the agreed CI test strategy without creating CI configuration, and `TASK-007B` implements it sequentially. After `TASK-007B`, `TASK-008` and `TASK-009` may proceed while file ownership remains disjoint. `TASK-010A` exclusively owns its migration, database tests, fixtures/helpers, and generated types through green CI evidence; it may run concurrently with `TASK-009`, which exclusively owns global styles and root layout. `TASK-010B` follows `TASK-010A` and owns login integration and server configuration. Execute `TASK-011` only after `TASK-008` and `TASK-010B`. No TASK-010 work may edit TASK-009 presentation files. Vercel does not apply Supabase migrations: apply the reviewed limiter migration to each target Supabase project before deploying application code that requires it.
...
- [x] TASK-009 — Tokonic design tokens
- [x] TASK-010A — PostgreSQL login rate-limit contract
- [x] TASK-010B — Login rate-limit integration
- [x] TASK-010C — Login rate-limit audit remediation
- [x] TASK-011 — Runtime seller RLS
...
| TASK-008 | Dependencies: TASK-007B. Files/areas: new environment-driven script under `scripts/`, server-only environment parsing used by the script, `.env.example`, `supabase/seed.sql` if sample non-secret catalog-independent data remains necessary. Done criteria: one command reads required synthetic admin email/password and seller fields from CI environment variables, creates or updates the Auth user in the CI-hosted disposable stack, obtains its generated ID at runtime, upserts exactly one linked seller row, is idempotent, rejects the shared remote project and non-CI targets by default, prints no password/token/key, and passes dry-run plus repeated-execution tests in CI with the green workflow run URL recorded. Test-first critical path: yes, validation and repeat execution against CI-hosted disposable Supabase. Parallel safety: safe in batch 1A after TASK-007B if no workflow/package/config edits are made; coordinate `.env.example` ownership. Evidence: https://github.com/teggar4ar/tokonic/actions/runs/30070026752/job/89408773211 (`15a834f`). | ✅ | 2026-07-24 |
| TASK-009 | Dependencies: TASK-007B. Files/areas: `src/app/globals.css`, root layout font configuration, shared UI primitives, login presentation. Done criteria: semantic Tokonic colors, spacing, radius, focus, tabular-number, and Figtree typography tokens replace grayscale/Arial defaults; buyer controls support 44px targets; login remains functional without becoming the template for later screens; lint, typecheck, test, and build pass. Test-first critical path: no; perform visual/accessibility checks at 360, 768, 1024, and 1440 px. Parallel safety: safe in batch 1A with exclusive ownership of global styles and root layout. | ✅ | 2026-07-24 |
| TASK-010A | Dependencies: TASK-007B. Files/areas: one ordered migration under `supabase/migrations/`, `tests/integration/database/login-rate-limit.test.ts`, deterministic fixtures/helpers, `src/lib/supabase/database.types.ts`, and the generated-types check/artifact steps in `.github/workflows/ci.yml`. Done criteria: failing database tests are written first; an atomic PostgreSQL operation enforces two independent keyed-digest buckets (canonical trusted client IP and normalized email), each allowing 5 attempts per 15 minutes under concurrent requests; no raw email, password, session data, raw client IP, or forwarding-header chain is stored; limiter state is durable across Vercel instances; the table has RLS enabled and no direct browser/`anon`/`authenticated` access; function execution is revoked from `PUBLIC`, `anon`, and `authenticated`, granted only to the narrow trusted server role, and any `SECURITY DEFINER` function resides in a non-exposed schema with a safe `search_path` and fully qualified objects; successful-login support can delete only the email bucket; an idempotent manual operator cleanup removes rows expired for more than 48 hours without PII logging; empty migration replay, database lint, generated-type review, advisors, and database integration tests pass in the CI-hosted disposable stack; the green workflow run URL is recorded before remote deployment. Rotating the dedicated digest secret intentionally invalidates existing buckets. Test-first critical path: yes. Parallel safety: exclusive migration/database-fixture/generated-types task, with a narrowly approved serialized exception for only the CI generated-types stale check and artifact upload; no other task may edit the workflow during this completion; may run concurrently with TASK-009 only while file ownership remains disjoint. Evidence: https://github.com/teggar4ar/tokonic/actions/runs/30076650889 (`ad33f57`). | ✅ | 2026-07-24 |
| TASK-010B | Dependencies: TASK-010A. Files/areas: login boundary, server-only rate-limit data adapter/service, current Vercel trusted-client-identity contract verification, server environment schema, `.env.example`, and `tests/integration/services/`. Done criteria: current official Vercel documentation is used to select and record the trusted client-IP header contract before coding; missing or untrusted client identity fails closed; the login service derives dedicated-secret keyed digests for canonical client IP and normalized email, consumes both PostgreSQL buckets before Supabase Auth, and deletes only the email bucket after successful login; invalid input, throttling, invalid credentials, missing/malformed configuration, and database/function failure produce the same generic public result; deterministic service tests cover ordering, allowed/throttled/reset behavior mapping, dependency failure, trusted-identity rejection, and generic responses; no state relies on Vercel memory or writable files; remote leaked-password protection remains a deployment checklist action. Configuration is fixed to 5 attempts per 15 minutes, uses a dedicated high-entropy server-only digest secret, and has no permissive fallback. Test-first critical path: yes. Parallel safety: runs after TASK-010A; must not edit TASK-009 presentation files or migration/generated-type artifacts. Evidence: https://github.com/teggar4ar/tokonic/actions/runs/30081482977 (`5525d75`). Approved follow-up: explicit `localhost-development` identity mode is limited to non-production without Vercel, ignores all request headers, and uses fixed loopback identity `127.0.0.1`; `vercel-direct` remains unchanged. Follow-up evidence: local focused service tests plus lint, typecheck, unit tests, and build on 2026-07-24; no database or workflow change. | ✅ | 2026-07-24 |
| TASK-010C | Dependencies: TASK-010A, TASK-010B. Files/areas: `.github/workflows/ci.yml`, `tokonic-technical-design-document.md` environment and trusted-identity sections, `docs/security-guidelines.md`, and `docs/adr/001-vercel-direct-client-ip.md`. Done criteria: the disposable Supabase CI job runs the official Supabase Splinter advisor SQL after empty migration replay, emits security/performance findings in the workflow log, and fails on external `ERROR` or `WARN` findings; a dated ADR records the current official Vercel client-IP citation, URL, retrieval date, and paraphrased `vercel-direct` trust decision and is referenced from the TDD; the TDD environment list uses the implemented `LOGIN_RATE_LIMIT_ATTEMPTS` name with no `LOGIN_RATE_LIMIT_MAX_ATTEMPTS` variant; the security guideline accurately records the implemented PostgreSQL limiter while retaining genuinely open deployment actions; local validation and a pushed green CI run pass, with its URL recorded as evidence. Test-first critical path: no, documentation/CI-configuration remediation. Parallel safety: sequential; touches shared CI configuration and cross-referenced documentation. Evidence: https://github.com/teggar4ar/tokonic/actions/runs/30086963192 (`6513ec7`). | ✅ | 2026-07-24 |
| TASK-011 | Dependencies: TASK-008, TASK-010B. Files/areas: `tests/integration/database/seller-rls.test.ts`, Auth/seller fixtures, `requireAdmin()` integration coverage. Done criteria: the CI-hosted stack replays migrations from empty; runtime tests prove anon cannot read seller data, owner can read/update only its row, unrelated authenticated user reads/mutates zero rows, and protected data access rejects no-session/unrelated users; static SQL assertions are not the sole evidence; the green workflow run URL is recorded. Test-first critical path: yes, RLS isolation. Parallel safety: sequential database acceptance gate using exclusive disposable CI database state. Evidence: https://github.com/teggar4ar/tokonic/actions/runs/30088652557 (`fe45f1a`). | ✅ | 2026-07-24 |
...
- [x] TASK-012 — Product schema and RLS
- [x] TASK-013 — Product image Storage
...
- **ALT-015**: Keep login counters in a process-local `Map` or writable runtime file. Rejected because Vercel instances do not share durable memory, cold starts reset counters, concurrent instances can bypass the limit, and the TDD forbids reliance on instance memory or writable runtime files.
- **ALT-016**: Run limiter tests against the shared remote Supabase project or require local Docker. Rejected because database and concurrency tests are restricted to the CI-hosted disposable stack.
...
- **TEST-026**: Login limiter database tests issue deterministic and genuinely overlapping consume operations against the disposable Supabase stack and prove two-bucket allowed attempts, atomic throttling, 15-minute reset, key isolation, successful-login email-bucket deletion, restricted grants, and idempotent cleanup of rows expired for more than 48 hours. Service tests prove trusted-client identity handling, keyed-digest derivation boundaries, generic public responses, operation ordering, and fail-closed behavior for malformed configuration and database failure. Later endpoint tasks separately test shipping, checkout/payment creation, and lookup rate limits.
...
- **RISK-015**: Spoofed or unstable forwarding headers could evade limits or make unrelated users share a bucket; mitigation is implementation-blocking verification of the current Vercel trusted-client-identity contract, fail-closed handling when it is absent or untrusted, canonicalization, keyed irreversible digests, and no raw identity storage or logging.
- **RISK-016**: Deploying application code before its limiter migration could bypass protection or lock out login; mitigation is fail-closed application behavior, green disposable-CI evidence, and migration-before-application deployment ordering.
